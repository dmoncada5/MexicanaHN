...
PermisoEPago:any;
  ...
  getFormapagos(){
     ...
     getNumerion(){
      ...
      this.facturaService.getnumeracion('/factura/correlativo', user.company, 'Entrega').subscribe(
          (res) => {
           //   this.FacturaE.DocNum=res[0]['Correlativo'];
             this.series = res; 
     ...
     numerosuc(event){
      ...
      const user = JSON.parse(localStorage.getItem('usuario'));
      this.facturaService.getOnenumeracion('/factura/correlativoOne', user.company, 'Entrega', event).subscribe(
          (res1) => {
    ...
    save() {
      ...
      this.guardarValidaciones('Lasagna', (valida: valida[]) => {
          ...
          if (!validaciones) {
              ...
              this._matSnackBar.open('Uno de los productos recae en inventario negativo o no existe en bodega! '+this.ELEMENT_DATA[index]['itemCode'] , 'OK',  {
                  verticalPosition: 'top',
                  duration: 2000
              });
  ...
  guardarValidaciones = (meal, callback) => {
      ...
      for (let index = 0; index < this.ELEMENT_DATA.length; index++) {
          ...
          this.facturaService.getExistencia('/products/Existencia', this.ELEMENT_DATA[index]['itemCode'], this.ELEMENT_DATA[index]['almacen']).subscribe(
              (res: any[]) => {
                  if (res.length === 0){
