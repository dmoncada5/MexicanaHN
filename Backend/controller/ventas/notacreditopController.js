...
const fs = require('fs');
var rawdata = fs.readFileSync('././Querys/ventas/QueryNotaPCredito.json');
...
class MainController {
    ...
    async getAllNC(req, res) {

        try {
            const pool = await poolPromise
            const result = await pool.request()
                .query(queries.getAllDataNC)
            res.json(result.recordset)
        } catch (error) {
            res.status(500)
            res.send(error.message)
        ...
        try {
            if (req.body.SocioCode != null) {
                ...
                .input('Serie', sql.NVarChar, req.body.Serie)
                    ...
                    .query(queries.addNewNotaCreditoEncabezado)
                res.json(result)
            } else {
        ...
        try {
            if (req.body.DocNum != null && req.body.itemCode != null) {
                ...
                .input('DocNum', sql.NVarChar, req.body.DocNum)
                    ...
                    .input('totaLine', sql.Numeric(7, 2), req.body.totaLine)
                    .input('almacen', sql.NVarChar, req.body.almacen)

                .query(queries.addNewNotaCreditoDetalle)
                res.json(result)
            } else {
                res.send('Please fill all the details!')
            }
    ...
    async updateDataeEncabezado(req, res) {
        ...
        try {
            ...
            const result = await pool.request()
                ...
                .input('tipo', sql.NVarChar, req.body.tipo)
                .input('RTN', sql.NVarChar, req.body.RTN)
                .query(queries.updateNotaCreditoEncabezado)
            res.json(result)
    ...
    async updateDataDetalle(req, res) {
        ...
        try {
            ...
            .query(queries.updateNotaCreditoDetalle)
            res.json(result)
        ...
        try {
            ...
            if (DocNum != null) {
                ...
                const result = await pool.request()
                    ...
                    .query(queries.deleteNotaCreditoDetalle)
                res.json(result)
            } else {
                res.send('Please fill all the details!')
            }
